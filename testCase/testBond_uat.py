...
from testData.data import bondPool_uat,bondPool_uat,bondAcc
from config import bossAcc

# @unittest.skip('跳过')
@ddt.ddt
class TestBond_uat(ParameTestCase):
	@classmethod
	def setUpClass(cls):
		logging.info(' ########## 债券下单测试开始 ########## ')
	...
	def test_Bond_01(self,productCode):
		...
		self.__class__.pageBond.select_custRatioRisk()
		self.__class__.pageBond.input_middleSuggest()
		self.__class__.pageBond.input_productMayRisk()
	...
	def test_Bond_02(self,productCode):
		...
		self.__class__.pageBond.click_result()
		time.sleep(3)
		amount=self.__class__.pageBond.input_units()
